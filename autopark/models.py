...
class Brand(models.Model):
    brand_name = models.CharField(
        max_length=70, help_text="Введите название бренда", verbose_name="Бренд"
    )
    type = models.CharField(
        max_length=70, help_text="Введите тип техники", verbose_name="Тип"
    )
    tank_volume = models.IntegerField(
        validators=[MinValueValidator(0)], verbose_name="Объем бака"
    )
    seats_number = models.IntegerField(
        validators=[MinValueValidator(0)], verbose_name="Количество мест"
    )
    load_capacity = models.IntegerField(
        validators=[MinValueValidator(0)], verbose_name="Грузоподъемность"
    )
...
class Enterprise(models.Model):
    name = models.CharField(
        ...
        verbose_name="Имя предприятия",
    )
    address = models.CharField(
        max_length=70,
        help_text="Введите адрес предприятия",
        verbose_name="Адрес предприятия",
    )
    num_of_employee = models.IntegerField(
        validators=[MinValueValidator(0)], verbose_name="Число сотрудников"
    )
...
class Vehicle(models.Model):
    ...
    price = models.IntegerField(
        ...
        help_text="Введите стоимость в рублях",
        verbose_name="Стоимость",
    )
    brand = models.ForeignKey(
        Brand, verbose_name="Бренд", on_delete=models.SET_NULL, null=True
    )
    owner = models.ForeignKey(
        Enterprise, verbose_name="Владелец", on_delete=models.SET_NULL, null=True
    )
    ...
    condition = models.CharField(
        choices=CONDITION, max_length=7, default="UNKNOWN", verbose_name="Состояние"
    )
    ...
    def save(self, *args, **kwargs):
        if self.date is None:
            self.date = datetime.datetime.now().replace(
                tzinfo=pytz.timezone(self.owner.zoner)
            )
        else:
...
class Driver(models.Model):
    name = models.CharField(
        max_length=70, help_text="Введите ФИО", verbose_name="Имя сотрудника"
    )
    # date_of_birthday = models.DateField(verbose_name='Дата рождения')
    residential_address = models.TextField(
        help_text="Введите адрес места проживания", verbose_name="Адрес"
    )
    ...
    car = models.ForeignKey(
        Vehicle, verbose_name="Машина", on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    status = models.CharField(
        choices=STATUS, max_length=4, default="NACT", verbose_name="Статус", null=True
    )
...
class Routes(models.Model):
    ...
    timestamp = models.DateTimeField(
        auto_now_add=True
    )  # created_time  изменить название переменной
    # time_zone = models.TimeField(default=pytz.UTC)
    objects = models.Manager()
    distance = models.IntegerField(default=0)
    max_speed = models.IntegerField(default=0)
    step = models.IntegerField(default=0)
    start = models.CharField(max_length=120, default="")
    finish = models.CharField(max_length=120, default="")
